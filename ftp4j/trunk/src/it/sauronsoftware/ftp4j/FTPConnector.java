...
public abstract class FTPConnector {
	 ...
	 * @since 1.7
	 */
	private UnifiedSocket connectingCommunicationChannelSocket;
	 ...
	 * @since 1.7
	 */
	protected UnifiedSocket tcpConnectForCommunicationChannel(String host, int port) throws IOException {
		try {
			connectingCommunicationChannelSocket = new UnifiedSocket();
	 ...
	 * @since 1.7
	 */
	protected UnifiedSocket tcpConnectForDataTransferChannel(String host, int port) throws IOException {
		UnifiedSocket socket = new UnifiedSocket();
		socket.setSoTimeout(readTimeout * 2000);
...
System.out.println("Connect to: " + host + " port: " + port);
		socket.connect(new InetSocketAddress(host, port), connectionTimeout * 2000);
		return socket;
	}
	protected UnifiedSocket tcpConnectForDataTransferChannel(String host, int port, String localHost, int localPort) throws IOException {
                // Not yet Implemented
System.out.println("Not ready for use!!");
		return null;
	}

	public UnifiedSocket udtConnectForDataTransferChannelUDT(String host, int port)
			throws IOException, UnknownHostException {
            
                UnifiedSocket socket = null;
                try {
                    socket = new UnifiedSocket(host, port, false);
	...
	public UnifiedSocket udtConnectForDataTransferChannelUDT(String host, int port, long size)
			throws IOException, UnknownHostException {
            
                UnifiedSocket socket = null;
                InetAddress address=InetAddress.getByName(host);
                socket = new UnifiedSocket(address, port, size);
	...
	public void abortConnectForCommunicationChannel() {
        ...
        public UnifiedSocket getSocket() {
	 ...
	 */
	public abstract UnifiedSocket connectForCommunicationChannel(String host, int port)
			throws IOException;
	 ...
	 *             If the connection cannot be established.
	 */
	public abstract UnifiedSocket connectForDataTransferChannel(String host, int port)
			throws IOException;
	public abstract UnifiedSocket connectForDataTransferChannelUDT(String host, int port)
			throws IOException;
	public abstract UnifiedSocket connectForDataTransferChannelUDT(String host, int port, long size) // Deprecated!
			throws IOException;
