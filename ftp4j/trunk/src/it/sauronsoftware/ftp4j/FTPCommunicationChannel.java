...
public class FTPCommunicationChannel {
	 ...
	 * The connection.
	 */
	private UnifiedSocket connection = null;
	 ...
	 */
	public FTPCommunicationChannel(UnifiedSocket connection, String charsetName)
			throws IOException {
	...
	public void ssl(SSLSocketFactory sslSocketFactory) throws IOException {
		...
		connection = new UnifiedSocket(sslSocketFactory.createSocket(connection.getTCPSocket(), host, port, true));
		InputStream inStream = connection.getInputStream();
		OutputStream outStream = connection.getOutputStream();
		reader = new NVTASCIIReader(inStream, charsetName);
		writer = new NVTASCIIWriter(outStream, charsetName);
