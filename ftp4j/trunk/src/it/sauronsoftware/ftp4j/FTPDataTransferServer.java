 ...
 */
public class FTPDataTransferServer implements FTPDataTransferConnectionProvider,
		Runnable {

	/**
	 * The ServerSocket object waiting for the incoming connection.
	 */
	private UnifiedServerSocket serverSocket = null;

	/**
	 * The socket to be established with the remote host.
	 */
	private UnifiedSocket socket;
	...
	public FTPDataTransferServer() throws FTPDataTransferException {
		...
		if (useRange) {
			...
			while (!done && (size = availables.size()) > 0) {
				...
				try {
					serverSocket = new UnifiedServerSocket();
